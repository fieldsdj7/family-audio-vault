...
const upcomingTools: ToolCard[] = [
  { title: 'Needs Review', description: 'One simple list for recordings that need a transcript checked, a family story reviewed, or more information added.', icon: FileWarning, status: 'Planned next' },
  { title: 'Trash & Restore', description: 'Replace permanent deletion with a safe holding area, so a recording can be restored if something is removed by mistake.', icon: Trash2, status: 'Planned' },
  { title: 'Split Recording', description: 'Keep one original recording safe while separating multiple short question answers into their own organized entries.', icon: Scissors, status: 'Planned' },
...
export default function ProjectToolsPage() {
  ...
  useEffect(() => {
    async function checkAccess() {
      ...
      if (!session) { setIsAdmin(false); setCheckingAccess(false); return; }
      const { data, error } = await supabase.from('vault_admins').select('user_id').eq('user_id', session.user.id).maybeSingle();
