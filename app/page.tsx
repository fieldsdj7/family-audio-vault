...
function VaultAudioPlayer({
  ...
  const clipEnd =
    typeof track.clip_end_seconds === "number" &&
    track.clip_end_seconds > clipStart
  ...
  function prepareAudio() {
    ...
    if (!audio) return;
    const actualEnd = clipEnd
      ? Math.min(clipEnd, audio.duration)
      : audio.duration;
    setPlayableDuration(Math.max(0, actualEnd - clipStart));
    audio.currentTime = Math.min(clipStart, audio.duration || clipStart);
  ...
  function updatePosition() {
    ...
    const actualEnd = clipEnd
      ? Math.min(clipEnd, audio.duration)
      : audio.duration;
    const duration = Math.max(0, actualEnd - clipStart);
    setPlayableDuration(duration);
    setCurrentSeconds(
      Math.min(Math.max(0, audio.currentTime - clipStart), duration),
    );

    if (clipEnd && audio.currentTime >= actualEnd - 0.05) {
      audio.pause();
      audio.currentTime = actualEnd;
  ...
  async function togglePlayback() {
    ...
    if (playableDuration > 0 && currentSeconds >= playableDuration - 0.05) {
  ...
  function seek(nextSeconds: number) {
    const audio = audioRef.current;
    if (!audio) return;
    const bounded = Math.max(0, Math.min(nextSeconds, playableDuration));
    audio.currentTime = clipStart + bounded;
    setCurrentSeconds(bounded);
  }
  ...
  return (
    <div className="mt-7 rounded-2xl border border-stone-200 bg-white p-4">
      <audio
        ref={audioRef}
        key={track.id}
        src={track.playback_url}
        preload="metadata"
        onLoadedMetadata={prepareAudio}
      ...
      <div className="flex items-center gap-4">
        ...
        <input
          ...
          step="0.1"
          value={Math.min(currentSeconds, playableDuration || 0)}
        ...
        <span className="shrink-0 text-sm tabular-nums text-stone-600">
          ...
          {formatAudioTime(playableDuration)}
        </span>
      </div>
      {track.source_track_id && (
        <p className="mt-3 text-xs text-stone-500">
          This answer plays only its saved section of the original recording.
        </p>
      )}
    </div>
  );
...
export default function Home() {
  ...
  const availableVaults = vaults.filter((vault) =>
    accessibleVaults.includes(vault.name),
  );
  const currentVault =
    availableVaults.find((vault) => vault.name === activePerson) ||
    availableVaults[0];
    ...
    if (!userIsAdmin) {
      ...
      permittedVaults = error
        ...
        : (data || [])
            ...
            .filter((name): name is VaultName =>
              vaults.some((vault) => vault.name === name),
            );
    }
  ...
  async function fetchTracks(preferredPerson: VaultName) {
    ...
    // Once a long recording has answer entries, keep its untouched master
    // available in Admin/Split Recording but out of the family-facing list.
    const visibleTracks = (data || []).filter(
      (track: AudioTrack) => track.is_split_master !== true,
    );

    const tracksWithSecureLinks = await Promise.all(
      visibleTracks.map(async (track: AudioTrack) => {
        ...
        if (signedUrlError) {
          console.error(
            `Could not create playback link for ${track.id}:`,
            signedUrlError,
          );
          return track;
        }
  ...
  useEffect(() => {
    ...
    // This subscription is intentionally created only once for the page.
  ...
  async function trashTrack(track: AudioTrack) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.error || "The recording could not be moved to Trash.",
        );
      }
      ...
      setSelectedTrack((current) =>
        current?.id === track.id
          ? remainingTracks.find(
              (item) => (item.vault_person || "Dad") === activePerson,
            ) || null
          : current,
      );
      setIsPlaying(false);
  ...
  async function downloadTrack(track: AudioTrack) {
    ...
    if (!storagePath) {
      window.alert(
        "This older recording does not have a downloadable audio file linked to it.",
      );
      return;
    }
    ...
    try {
      ...
      if (error || !data?.signedUrl) {
        throw new Error(
          error?.message ||
            "The audio file could not be prepared for download.",
        );
      }
      ...
      <main className="flex min-h-screen items-center justify-center bg-[#f6f0e5] p-5 text-stone-800">
        <div className="w-full max-w-md rounded-3xl border border-stone-300 bg-[#fffaf0] p-8 text-center shadow-xl">
          ...
          <p className="mt-3 text-sm leading-relaxed text-stone-600">
            This account has not been assigned to a family vault. Please ask an
            administrator to add access.
          </p>
          <button
        ...
        <aside className="min-w-0 overflow-hidden border-b border-stone-800 bg-[#20221e] px-5 py-6 text-stone-100 lg:border-b-0 lg:border-r">
          ...
          <nav className="mt-8 flex max-w-full min-w-0 gap-2 overflow-x-auto lg:block lg:space-y-2">
            {availableVaults.map((vault) => (
              <button
                ...
                className={`flex w-auto shrink-0 items-center gap-3 whitespace-nowrap rounded-xl px-4 py-3 text-left text-sm transition lg:w-full lg:whitespace-normal ${activePerson === vault.name ? "bg-white/10 font-medium text-white" : "text-stone-300 hover:bg-white/10 hover:text-white"}`}
              >
          ...
          <div className="mt-10 hidden border-t border-white/10 pt-6 lg:block">
            ...
            <p className="mt-6 border-l-2 border-[#d8a95f] pl-4 font-serif text-lg leading-relaxed text-stone-200">
              “A people without knowledge of their past is like a tree without
              roots.”
            </p>
          </div>
        </aside>
              ...
              <div className="grid md:grid-cols-[1.05fr_.95fr]">
                <div className="flex min-h-[280px] flex-col justify-between p-7 text-[#fffaf0] md:p-10">
                  ...
                  <div className="mt-8 flex items-center gap-3 text-sm text-stone-200">
                    ...
                    <span>
                      {personTracks.length}{" "}
                      {personTracks.length === 1 ? "recording" : "recordings"}{" "}
                      preserved
                    </span>
                  </div>
                </div>
              ...
              <section className="mt-10 rounded-3xl border border-stone-300 bg-[#fffaf0] p-6 shadow-sm md:p-8">
                <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    {getStoragePath(selectedTrack) &&
                      !selectedTrack.source_track_id && (
                        <button
                          type="button"
                          onClick={() => void downloadTrack(selectedTrack)}
                          disabled={downloadingTrackId === selectedTrack.id}
                          className="inline-flex items-center gap-2 rounded-xl border border-[#d1af77] bg-[#fff7e8] px-3 py-2 text-sm font-semibold text-[#744b1e] transition hover:bg-[#f8e6c2] disabled:cursor-not-allowed disabled:opacity-60"
                        >
                          <Download className="h-4 w-4" />
                          {downloadingTrackId === selectedTrack.id
                            ? "Preparing…"
                            : "Download audio"}
                        </button>
                      )}
                    {isAdmin && (
                      <button
                        type="button"
                        onClick={() => void trashTrack(selectedTrack)}
                        disabled={trashingTrackId === selectedTrack.id}
                        className="inline-flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-900 transition hover:bg-amber-100 disabled:cursor-not-allowed disabled:opacity-60"
                      >
                        <Trash2 className="h-4 w-4" />
                        {trashingTrackId === selectedTrack.id
                          ? "Moving…"
                          : "Move to Trash"}
                      </button>
                    )}
                ...
                {selectedTrack.playback_url ? (
                  ...
                  <p className="mt-7 rounded-xl bg-rose-50 p-4 text-sm text-rose-800">
                    This recording could not be opened. Please refresh the page
                    and try again.
                  </p>
                )}
                ...
                {selectedTrack.story_chapter && (
                  <div className="mt-7 border-t border-stone-200 pt-7">
                    ...
                    <h3 className="mt-2 font-serif text-2xl text-stone-900">
                      {selectedTrack.story_title || selectedTrack.title}
                    </h3>
              ...
              <section className="mt-10 rounded-3xl border border-dashed border-stone-300 bg-[#fffaf0] p-10 text-center">
                ...
                {!loading && (
                  <p className="mt-2 text-stone-600">
                    The first recording will become the beginning of this legacy
                    book.
                  </p>
                )}
              </section>
                ...
                <div className="mt-3 flex gap-2 overflow-x-auto pb-2">
                  {categories.map((category) => (
                    <button
                      ...
                      className={`shrink-0 rounded-full px-4 py-2 text-sm font-medium transition ${activeCategory === category ? "bg-[#3b4536] text-white" : "border border-stone-300 bg-[#fffaf0] text-stone-700 hover:border-[#a66b27]"}`}
                    >
                      {category}
                    </button>
                  ))}
              ...
              <div className="mt-5 space-y-3">
                {filteredTracks.map((track, index) => (
                  <button
                    ...
                    className={`group flex w-full items-center gap-4 rounded-2xl border p-4 text-left transition md:p-5 ${selectedTrack?.id === track.id ? "border-[#b57931] bg-[#f4e7cf]" : "border-stone-300 bg-[#fffaf0] hover:border-[#b57931] hover:shadow-sm"}`}
                  >
