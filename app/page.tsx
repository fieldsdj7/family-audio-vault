...
export default function Home() {
  ...
  async function deleteTrack(track: AudioTrack) {
    const confirmed = window.confirm(
      `Delete “${track.title}”?\n\nThis permanently removes the audio, transcript, and story from the vault. This cannot be undone.`
    );
    if (!confirmed) return;
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || 'The recording could not be deleted.');
      }
      ...
      window.alert(error instanceof Error ? error.message : 'The recording could not be deleted.');
    } finally {
      ...
      <div className="grid min-h-screen lg:grid-cols-[250px_minmax(0,1fr)]">
        <aside className="border-b border-stone-800 bg-[#20221e] px-5 py-6 text-stone-100 lg:border-b-0 lg:border-r">
          ...
          <nav className="mt-8 flex gap-2 overflow-x-auto lg:block lg:space-y-2">
            ...
            {isAdmin && (
              <a href="/admin" className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left text-sm text-stone-300 transition hover:bg-white/10 hover:text-white">
