...
type TrashAction = "trash" | "restore" | "permanent";
...
function isTrashAction(value: unknown): value is TrashAction {
  return value === "trash" || value === "restore" || value === "permanent";
}
  ...
  if (!member.isAdmin) {
    return {
      ...
      response: Response.json(
        { error: "Only a Vault administrator can manage Trash." },
        { status: 403 },
      ),
    };
  }
...
export async function POST(request: Request) {
  try {
    ...
    if (!isTrashAction(body.action)) {
      return Response.json(
        { error: "That Trash action is not available." },
        { status: 400 },
      );
    }
    ...
    if (body.action === "trash") {
      if (recording.trashed_at) {
        return Response.json(
          { error: "This recording is already in Trash." },
          { status: 409 },
        );
      }
      ...
      await db
        ...
        .bind(access.member.email, recording.id)
        .run();
    ...
    if (body.action === "restore") {
      if (!recording.trashed_at) {
        return Response.json(
          { error: "This recording is not in Trash." },
          { status: 409 },
        );
      }
    ...
    const linkedRecording = await db
      .prepare("SELECT id FROM audio_tracks WHERE source_track_id = ? LIMIT 1")
      .bind(recording.id)
      .first<{ id: string }>();
    ...
    const photos = await db
      .prepare("SELECT storage_path FROM story_photos WHERE audio_track_id = ?")
      .bind(recording.id)
      .all<PhotoRow>();
    const storagePaths = [
      recording.storage_path,
      ...photos.results.map((photo) => photo.storage_path),
    ].filter((path): path is string => Boolean(path));

    if (storagePaths.length > 0) {
      await files.delete(storagePaths);
    }

    await db.prepare("DELETE FROM audio_tracks WHERE id = ?").bind(recording.id).run();

    return Response.json({ permanentlyDeleted: true });
  } catch (error) {
    return vaultAccessResponse(error);
  }
}
