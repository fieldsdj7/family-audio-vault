...
export async function POST(request: Request) {
  ...
  try {
    ...
    const instructions = [
      ...
      "For an incomplete source collection, keep the proposed outline compact. Prefer about 6 to 10 broad working chapters unless the source material clearly requires fewer.",
      "Do not increase the chapter count simply because more topics are detected. Related childhood memories should usually be grouped into broader chapters with multiple themes.",
      "A place should appear in the places list only if it is meaningfully connected to the subject's own life story, such as where he lived, worked, served, attended school, traveled in a personally significant way, or where an important life event occurred.",
      "Do not list incidental places that only appear in someone else's story, business history, ownership detail, or passing reference unless that place materially affected the subject's life.",
      "Only flag a contradiction when two source statements make directly incompatible factual claims about the same event, person, date, age, place, relationship, or outcome.",
      "Do not flag differences in wording, level of detail, approximate ages or dates, omitted details, or compatible versions as contradictions. Treat those as complementary unless they truly cannot both be correct.",
      "Repeated or overlapping memories should be grouped aggressively when they concern the same person, event, relationship, object, place, or recurring anecdote, even if the interview questions were different.",
      "When repeated memories contain compatible details, combine the details and cite all supporting recording IDs rather than proposing separate chapters for each version.",
